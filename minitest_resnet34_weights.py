...
def plot_statistics_by_layer_size(arch, statistical_data, layer_size_list):
    ...
    plt.title("Noise effect by layer size for ResNet34 architecture")
...
def plot_statistics_by_layer_location(arch, statistical_data):
    ...
    path = "RESNET34/OUTPUT"
    if not os.path.exists(path):
        os.mkdir(path)
        
    plt.title("Noise effect by layer location for ResNet34 architecture")
    plt.ylabel("Mean Value")
...
def plot_weights_distribution():
	output, ideal_weights, analog_weights = [], [], []
	path_ideal_weights = "~/aihwkit/minitest_plot_resnet34_weights_per_layer/ideal_weights/"
	path_analog_weights = "~/aihwkit/minitest_plot_resnet34_weights_per_layer/analog_weights/"
	
	ideal_weights_l1 = torch.load(path_ideal_weights + "layer_1.pth")
	ideal_weights_l10 = torch.load(path_ideal_weights + "layer_10.pth")
	ideal_weights_l20 = torch.load(path_ideal_weights + "layer_20.pth")
	ideal_weights_l37 = torch.load(path_ideal_weights + "layer_37.pth")
	
	ideal_weights.append(ideal_weights_l1)
	ideal_weights.append(ideal_weights_l10)
	ideal_weights.append(ideal_weights_l20)
	ideal_weights.append(ideal_weights_l37)
	
	analog_weights_l1 = torch.load(path_analog_weights + "layer_1.pth")
	analog_weights_l10 = torch.load(path_analog_weights + "layer_10.pth")
	analog_weights_l20 = torch.load(path_analog_weights + "layer_20.pth")
	analog_weights_l37 = torch.load(path_analog_weights + "layer_37.pth")
	
	analog_weights.append(analog_weights_l1)
	analog_weights.append(analog_weights_l10)
	analog_weights.append(analog_weights_l20)
	analog_weights.append(analog_weights_l37)
	
	output.append(ideal_weights)
	output.append(analog_weights)
	
	plt.hist(output,bins=100)
	plt.legend(['Analog Weights','Ideal Weights'])
	
	plt.savefig('resnet34_cifar10_weight_ditribution.png')
...
if __name__ == '__main__':
    ...
    count = 0    
    ideal_weight_dir_path = "ideal_weights/"
    analog_weight_dir_path = "analog_weights/"
    ...
    for name, layer in model.named_children():
      if isinstance(layer, nn.Conv2d):
        ...
        count = count + 1      
        ideal_weight = layer.weight
        #torch.save(ideal_weight,ideal_weight_dir_path + "layer_" + str(count) + ".pth")
        
        out_ideal = layer(data)
        
        layer_analog = conversion_to_analog(layer)
        
        analog_weight = layer_analog.get_weights()
        #torch.save(analog_weight,analog_weight_dir_path + "layer_" + str(count) + ".pth")
            ...
            if isinstance(block_layer, nn.Sequential):
              for l in block_layer:
                ...
                if isinstance(l, nn.Conv2d):
                  ...
                  analog_weight = l_layer_analog.get_weights()
                  #torch.save(analog_weight,analog_weight_dir_path + "layer_" + str(count) + ".pth")
              ...
              if isinstance(l, nn.Conv2d):
                ...
                analog_weight = analog_l.get_weights()
                #torch.save(analog_weight,analog_weight_dir_path + "layer_" + str(count) + ".pth")
    ...
    plot_statistics_by_layer_size('RESNET34',stat,layers_sizes)
    plot_statistics_by_layer_location('RESNET34',stat)
    """
    stat.clear()
