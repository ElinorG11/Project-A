...
def get_layer_size(current_layer):
    in_channels = current_layer.in_channels
    kernel_size = current_layer.kernel_size
    return in_channels * kernel_size[0] * kernel_size[1]
...
def calc_statistics(ideal_output, noisy_output):
    ...
    print("### Mean value is: " + str(errors_mean.item()) + " Standard deviation value is: " + str(
        errors_std.item()) + "\n")
...
def plot_statistics_per_architecture(arch, statistical_data):
    ...
    for arch in architectures:
        layer_size_list.append(torch.load(path + arch + "/layers_sizes.pth"))
    ...
    plt.figure(1)
    plt.title("Mean values of noise effect on the outputs for" + arch + "architectures")
    plt.ylabel("Mean Value")
    plt.scatter(x, m, s=area, c=colors, alpha=0.5)
    plt.grid(color='green', linestyle='--', linewidth=0.5)

    plt.figure(2)
    plt.title("Std values of noise effect on the outputs" + arch + "architectures")
...
def plot_statistics_for_all_architectures():
    ...
    for arch_idx in range(len(architectures)):
        ...
        for statistics in stat_list[arch_idx]:
            arch_m.append(statistics[0])
            arch_s.append(statistics[1])
        m.append(arch_m)
        s.append(arch_s)
...
if __name__ == '__main__':
    ...
    for architecture in architectures:
        ...
        # store all layer sizes for each model
        layer_sizes = []
        ...
        for layer in layers_list:
            ...
            layer_sizes.append(get_layer_size(layer))
        ...
        torch.save(stat, directory + architecture + "/statistical_data.pth")

        # save layer sizes in order to use them when creating the statistical plot later
        directory = "LAYER_SIZES/"
        if not os.path.exists(directory):
            os.mkdir(directory)

        directory = "LAYER_SIZES/" + architecture
        if not os.path.exists(directory):
            os.mkdir(directory)

        torch.save(layer_sizes, directory + "layers_sizes.pth")
